...
drop procedure if exists api_testor_suite;
delimiter $$
create procedure api_testor_suite( in p_token varchar(36), out p_id bigint, in p_code varchar(640) )
...
begin
  ...
  set v_code = 'api_testor_suite';
  set v_input_text = concat( 'token: ', testor_escape(p_token), '\n', 'code: ', testor_escape(p_code), '\n' );
  set v_input_json = concat( '{"token": "', testor_escape(p_token), '", "code": "', testor_escape(p_code), '"}' );
  ...
  if v_ready = 1 then
    ...
    set v_output = json_extract( testor_unescape(v_output_json), '$.suite_id' );
