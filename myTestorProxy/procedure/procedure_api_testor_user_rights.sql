...
begin
  ...
  if v_ready = 1 then
    ...
    set v_output = json_extract( testor_unescape(v_output_json), '$.api_call' );
    ...
    set v_output = json_extract( testor_unescape(v_output_json), '$.user_make' );
    if v_output is not null and v_output <> 'NULL' and v_output <> '\"NULL\"' then
      set v_output = replace( v_output, '\"', '' );
      set p_user_make = cast( v_output as signed );
    end if;
    set v_output = json_extract( testor_unescape(v_output_json), '$.user_demo' );
    if v_output is not null and v_output <> 'NULL' and v_output <> '\"NULL\"' then
      set v_output = replace( v_output, '\"', '' );
      set p_user_demo = cast( v_output as signed );
    end if;
    set v_output = json_extract( testor_unescape(v_output_json), '$.storage_full' );
    if v_output is not null and v_output <> 'NULL' and v_output <> '\"NULL\"' then
      set v_output = replace( v_output, '\"', '' );
