...
begin
  ...
  set v_input_text = concat( 'username: ', testor_escape(p_username), '\n', 'password: ', testor_escape(p_password), '\n' );
  set v_input_json = concat( '{"username": "', testor_escape(p_username), '", "password": "', testor_escape(p_password), '"}' );
  ...
  if v_ready = 1 then
    ...
    set v_output = json_extract( testor_unescape(v_output_json), '$.token' );
    if v_output is not null and v_output <> 'NULL' and v_output <> '\"NULL\"' then
      set v_output = replace( v_output, '\"', '' );
      set p_token = testor_unescape(v_output);
    end if;
  end if;
