...
drop procedure if exists api_testor_not_contains;
delimiter $$
create procedure api_testor_not_contains( in p_token varchar(36), out p_id bigint, in p_suite_id bigint, in p_case_id bigint, in p_code varchar(640), in p_operand varchar(8192), in p_value varchar(8192) )
...
begin
  ...
  set v_code = 'api_testor_not_contains';
  set v_input_text = concat( 'token: ', testor_escape(p_token), '\n', 'suite_id: ', p_suite_id, '\n', 'case_id: ', p_case_id, '\n', 'test_code: ', testor_escape(p_code), '\n', 'operand: ', testor_escape(p_operand), '\n', 'value: ', testor_escape(p_value), '\n' );
  set v_input_json = concat( '{"token": "', testor_escape(p_token), '", "suite_id": ', p_suite_id, ', "case_id": ', p_case_id, ', "test_code": "', testor_escape(p_code), '", "operand": "', testor_escape(p_operand), '", "value": "', testor_escape(p_value), '"}' );
