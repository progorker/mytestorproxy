...
begin
  ...
  set v_input_text = concat( 'token: ', testor_escape(p_token), '\n', 'right_code: ', testor_escape(p_right_code), '\n' );
  set v_input_json = concat( '{"token": "', testor_escape(p_token), '", "right_code": "', testor_escape(p_right_code), '"}' );
  ...
  if v_ready = 1 then
    ...
    set v_result_str = json_extract( testor_unescape(v_output_json), '$.result' );
