...
begin
  ...
  set v_input_text = concat( 'token: ', testor_escape(p_token), '\n', 'suite_id: ', p_suite_id, '\n', 'data: ', testor_escape(p_data), '\n', 'code: ', testor_escape(p_code), '\n', 'remove: ', p_remove, '\n' );
  set v_input_json = concat( '{"token": "', testor_escape(p_token), '", "suite_id": ', p_suite_id, ', "data": "', testor_escape(p_data), '", "code": "', testor_escape(p_code), '", "remove": ', p_remove, '}' );
  ...
  if v_ready = 1 then
    ...
    set v_output = json_extract( testor_unescape(v_output_json), '$.data' );
    if v_output is not null and v_output <> 'NULL' and v_output <> '\"NULL\"' then
      set v_output = replace( v_output, '\"', '' );
      set p_data = testor_unescape(v_output);
    end if;
  end if;
