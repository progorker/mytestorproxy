...
drop procedure if exists api_testor_test;
delimiter $$
create procedure api_testor_test( in p_token varchar(36), out p_id bigint, in p_suite_id bigint, in p_case_id bigint, in p_code varchar(640), in p_condition int, in p_message mediumtext )
...
begin
  ...
  set v_code = 'api_testor_test';
  set v_input_text = concat( 'token: ', testor_proxy_quote(p_token), '\n', 'suite_id: ', testor_proxy_quote(p_suite_id), '\n', 'case_id: ', testor_proxy_quote(p_case_id), '\n', 'test_code: ', testor_proxy_quote(p_code), '\n', 'condition: ', p_condition, '\n', 'message: ', testor_proxy_quote(p_message), '\n' );
  set v_input_json = concat( '{"token": "', testor_proxy_quote(p_token), '", "suite_id": ', p_suite_id, ', "case_id": ', p_case_id, ', "test_code": "', testor_proxy_quote(p_code), '", "condition": ', p_condition, ', "message": "', testor_proxy_quote(p_message), '"}' );
